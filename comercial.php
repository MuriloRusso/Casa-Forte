        ...
        <div class="full-width flex flex-wrap">
			...
			<?php 
				...
				<div class="container-posts flex justify-content-center flex-wrap">
					...
					<h1 class="text-center full-width">Comercial</h1>
				...
				$sql_code = "SELECT * FROM categorizar WHERE ID_categoria=3";	
				...
				if(!isset($_GET['page'])){
					...
					$sql_code = "SELECT * FROM categorizar WHERE ID_categoria=3 ORDER BY id DESC LIMIT 0, 10";
					...
					$sql_code = "SELECT * FROM categorizar WHERE ID_categoria=3 ORDER BY id DESC LIMIT ".$page."0, 10";
...
//					print 'id '.$row->ID.' cliente '.$row->ID_Cliente.' produto '.$row->ID_Produtos;
					...
					//	$numPages = ceil((intval($quantidadeProdutos / 10)));
						...
						for($a = 1; $a < $numPages+1; $a++){
							...
							if($a == $_GET['page'] || (!isset($_GET['page']) && $a == 1)){
								...
								print '<a href="comercial.php?page='.$a.'" class="btn-pagination-active">'.$a.'</a>';

							}

							else{

								print '<a href="comercial.php?page='.$a.'">'.$a.'</a>';
		...
		<script>
			...
			document.querySelector('#btn-category-3').href = '#';
		
			document.querySelector('#btn-category-3').click();
